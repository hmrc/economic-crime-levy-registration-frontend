...
object SecondContactEmailSummary {
  ...
  def row(emailAddress: Option[String])(implicit messages: Messages): Option[SummaryListRow] =
    emailAddress.map { answer =>
      SummaryListRowViewModel(
        key = Key("checkYourAnswers.secondContactEmail.label"),
        value = ValueViewModel(HtmlContent(HtmlFormat.escape(answer))),
        actions = Seq(
          ActionItemViewModel("site.change", contacts.routes.SecondContactEmailController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(
              messages("checkYourAnswers.secondContactEmail.label")
            )
        )
      )
    }
