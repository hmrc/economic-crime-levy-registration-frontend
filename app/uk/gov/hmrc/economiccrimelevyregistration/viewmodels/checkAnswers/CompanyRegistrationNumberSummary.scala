  ...
  def row()(implicit messages: Messages, request: RegistrationDataRequest[_]): Option[SummaryListRow] =
    request.registration.otherEntityJourneyData.companyRegistrationNumber.map { answer =>
      ...
      SummaryListRowViewModel(
        key = Key("checkYourAnswers.companyRegistrationNumber.label"),
        value = value,
        actions = Seq(
          ActionItemViewModel("site.change", routes.CompanyRegistrationNumberController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(
              messages("checkYourAnswers.companyRegistrationNumber.label")
            )
        )
      )
    }
