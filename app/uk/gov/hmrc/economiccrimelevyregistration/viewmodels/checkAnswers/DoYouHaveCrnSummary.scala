...
object DoYouHaveCrnSummary {
  ...
  def row()(implicit messages: Messages, request: RegistrationDataRequest[_]): Option[SummaryListRow] =
    request.registration.otherEntityJourneyData.hasUkCrn.map { answer =>
      val value = if (answer) "site.yes" else "site.no"

      SummaryListRowViewModel(
        key = Key("otherEntityType.crn.question.label"),
        value = ValueViewModel(value),
        actions = Seq(
          ActionItemViewModel("site.change", routes.DoYouHaveCrnController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(
              messages("otherEntityType.crn.question.label")
            )
        )
      )
    }
