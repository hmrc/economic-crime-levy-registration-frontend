...
object ExampleTextFieldSummary {

  def row(registration: Registration)(implicit messages: Messages): Option[SummaryListRow] =
    registration.exampleTextField.map { answer =>
      SummaryListRowViewModel(
        key = "exampleTextField.checkYourAnswersLabel",
        value = ValueViewModel(HtmlFormat.escape(answer).toString),
        actions = Seq(
          ActionItemViewModel("site.change", routes.ExampleTextFieldController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("exampleTextField.change.hidden"))
        )
      )
    }
}
