...
object ExampleYesNoSummary {

  def row(registration: Registration)(implicit messages: Messages): Option[SummaryListRow] =
    registration.exampleYesNo.map { answer =>
      val value = if (answer) "site.yes" else "site.no"

      SummaryListRowViewModel(
        key = "exampleYesNo.checkYourAnswersLabel",
        value = ValueViewModel(value),
        actions = Seq(
          ActionItemViewModel("site.change", routes.ExampleYesNoController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("exampleYesNo.change.hidden"))
        )
      )
    }
}
