...
object ExampleMultipleQuestionsSummary {

  def row(registration: Registration)(implicit messages: Messages): Option[SummaryListRow] =
    registration.exampleMultipleQuestions.map { answer =>
      val value = HtmlFormat.escape(answer.field1).toString + "<br/>" + HtmlFormat.escape(answer.field2).toString

      SummaryListRowViewModel(
        key = "exampleMultipleQuestions.checkYourAnswersLabel",
        value = ValueViewModel(HtmlContent(value)),
        actions = Seq(
          ActionItemViewModel("site.change", routes.ExampleMultipleQuestionsController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("exampleMultipleQuestions.change.hidden"))
        )
      )
    }
}
