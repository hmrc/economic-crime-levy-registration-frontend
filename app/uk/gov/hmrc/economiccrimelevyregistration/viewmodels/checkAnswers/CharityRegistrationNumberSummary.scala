  ...
  def row()(implicit messages: Messages, request: RegistrationDataRequest[_]): Option[SummaryListRow] =
    request.registration.otherEntityJourneyData.charityRegistrationNumber.map { answer =>
      ...
      SummaryListRowViewModel(
        key = Key("checkYourAnswers.charityRegistrationNumber.label"),
        value = value,
        actions = Seq(
          ActionItemViewModel("site.change", routes.CharityRegistrationNumberController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(
              messages("checkYourAnswers.charityRegistrationNumber.label")
            )
        )
      )
    }
