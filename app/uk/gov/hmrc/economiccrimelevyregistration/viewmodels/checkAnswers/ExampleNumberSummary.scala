...
object ExampleNumberSummary {

  def row(registration: Registration)(implicit messages: Messages): Option[SummaryListRow] =
    registration.exampleNumber.map { answer =>
      SummaryListRowViewModel(
        key = "exampleNumber.checkYourAnswersLabel",
        value = ValueViewModel(answer.toString),
        actions = Seq(
          ActionItemViewModel("site.change", routes.ExampleNumberController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("exampleNumber.change.hidden"))
        )
      )
    }
}
