...
object FirstContactNameSummary {
  ...
  def row(name: Option[String], secondContact: Boolean)(implicit messages: Messages): Option[SummaryListRow] =
    name.map { answer =>
      val contact = if (secondContact) "firstContactName" else "contactName"
