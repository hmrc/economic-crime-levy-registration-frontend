...
object DeregisterNumberSummary {

  def row(number: Option[String])(implicit messages: Messages): Option[SummaryListRow] =
    number.map { answer =>
      val value = ValueViewModel(HtmlContent(HtmlFormat.escape(answer)))

      SummaryListRowViewModel(
        key = Key("deregisterCya.t3.l4"),
        value = value,
        actions = Seq(
          ActionItemViewModel(
            "site.change",
