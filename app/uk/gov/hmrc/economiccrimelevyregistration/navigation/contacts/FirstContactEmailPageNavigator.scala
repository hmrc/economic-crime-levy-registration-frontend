...
class FirstContactEmailPageNavigator extends PageNavigator {
  ...
  override protected def navigateInNormalMode(eclRegistrationModel: EclRegistrationModel): Call =
    eclRegistrationModel.registration.contacts.firstContactDetails.emailAddress match {
      case Some(_) => contacts.routes.FirstContactNumberController.onPageLoad(NormalMode)
