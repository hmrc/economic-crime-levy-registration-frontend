...
import uk.gov.hmrc.economiccrimelevyregistration.models.{EclRegistrationModel, Mode, SessionKeys, TempLiabilityYear}
    ...
    (authorise andThen getRegistrationData).async { implicit request =>
      form
        ...
        .fold(
          ...
          liabilityDate => {
            val year           = TempTaxYear.fromDate(liabilityDate)
            ...
            val additionalInfo = request.additionalInfo.get.copy(
              ...
              liabilityYear = Some(TempLiabilityYear(year.startYear))
            )
