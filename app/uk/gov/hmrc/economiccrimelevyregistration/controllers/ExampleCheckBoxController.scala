...
import uk.gov.hmrc.economiccrimelevyregistration.pages.ExampleCheckBoxPage
...
class ExampleCheckBoxController @Inject() (
  override val messagesApi: MessagesApi,
  eclRegistrationConnector: EclRegistrationConnector,
  navigator: Navigator,
  authorise: AuthorisedAction,
  getRegistrationData: DataRetrievalAction,
  formProvider: ExampleCheckBoxFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: ExampleCheckBoxView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (authorise andThen getRegistrationData) { implicit request =>
    val preparedForm = request.registration.exampleCheckBox match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (authorise andThen getRegistrationData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value => {
          val updatedRegistration = request.registration.copy(exampleCheckBox = Some(value))

          eclRegistrationConnector.updateRegistration(updatedRegistration).map { registration =>
            Redirect(navigator.nextPage(ExampleCheckBoxPage, mode, registration))
          }
        }
      )
  }
