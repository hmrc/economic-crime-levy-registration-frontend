...
class FirstContactNameController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = (authorise andThen getRegistrationData) { implicit request =>
    Ok(view(form.prepare(request.registration.contacts.map(_.firstContactDetails.name))))
  }
      ...
      .fold(
        ...
        firstName =>
          eclRegistrationConnector
            .upsertRegistration {
              val updatedContacts: Option[Contacts] = for {
                contacts           <- request.registration.contacts
                firstContactDetails = contacts.firstContactDetails.copy(name = firstName)
                updatedContacts     = contacts.copy(firstContactDetails = firstContactDetails)
              } yield updatedContacts

              request.registration.copy(contacts = updatedContacts)
            }
