...
@Singleton
class DeregisterContactNameController @Inject() (
  val controllerComponents: MessagesControllerComponents,
  authorise: AuthorisedActionWithEnrolmentCheck,
  getDeregistrationData: DeregistrationDataRetrievalAction,
  deregistrationService: DeregistrationService,
  formProvider: DeregisterContactNameFormProvider,
  view: DeregisterContactNameView
      ...
      .fold(
        ...
        name => {
          ...
          (for {
            ...
            err => routeError(err),
            _ => toNextPage(mode, routes.DeregisterContactRoleController.onPageLoad(mode))
          )
        }
      )
  }
