...
class $className$ControllerSpec extends SpecBase {
  ...
  "onPageLoad" should {
    ...
    "populate the view correctly when the question has previously been answered" in new TestFixture(
      emptyRegistration.copy(??? = Some($className$.values.toSet)) // TODO Choose the data you are testing
    ) {
      val result: Future[Result] = controller.onPageLoad(NormalMode)(fakeRequest)

      status(result)          shouldBe OK
      contentAsString(result) shouldBe view(form.fill($className$.values.toSet), NormalMode)(fakeRequest, messages).toString
    }
    ...
    "redirect to the next page when valid data is submitted" in new TestFixture() {
      ...
      val result: Future[Result] =
        controller.onSubmit(NormalMode)(fakeRequest.withFormUrlEncodedBody("value[0]" -> $className$.values.head.toString))
    ...
    "return a Bad Request and errors when invalid data is submitted" in new TestFixture() {
      ...
      val formWithErrors: Form[Set[$className$]] = form.bind(Map("value[0]" -> ""))
