...
class AmlRegulatedPageNavigatorSpec extends SpecBase {
  ...
  "nextPage" should {
    "return a Call to the ??? page from the Aml regulated activity started in current financial year page in NormalMode when the 'Yes' option is selected" in forAll {
    ...
    "return a Call to the ??? page from the Aml regulated activity started in current financial year page in NormalMode when the 'No' option is selected" in forAll {
      registration: Registration =>
