...
class ExampleYesNoControllerSpec extends SpecBase {

  val view: ExampleYesNoView = app.injector.instanceOf[ExampleYesNoView]

  val formProvider = new ExampleYesNoFormProvider()
  ...
  class TestFixture(data: Registration = emptyRegistration) {
    val controller = new ExampleYesNoController(
  ...
  "onPageLoad" should {
    ...
    "populate the view correctly when the question has previously been answered" in new TestFixture(
      emptyRegistration.copy(exampleYesNo = Some(true))
    ) {
      val result: Future[Result] = controller.onPageLoad(NormalMode)(fakeRequest)
