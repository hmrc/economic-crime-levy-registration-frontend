...
import uk.gov.hmrc.economiccrimelevyregistration.models.{ExampleMultipleQuestions, NormalMode, Registration}
import uk.gov.hmrc.economiccrimelevyregistration.views.html.ExampleMultipleQuestionsView

import scala.concurrent.Future

class ExampleMultipleQuestionsControllerSpec extends SpecBase {

  val view: ExampleMultipleQuestionsView = app.injector.instanceOf[ExampleMultipleQuestionsView]

  val formProvider = new ExampleMultipleQuestionsFormProvider()

  val form: Form[ExampleMultipleQuestions] = formProvider()

  val mockEclRegistrationConnector: EclRegistrationConnector = mock[EclRegistrationConnector]

  class TestFixture(data: Registration = emptyRegistration) {
    val controller = new ExampleMultipleQuestionsController(
  ...
  "onPageLoad" should {
    ...
    "populate the view correctly when the question has previously been answered" in new TestFixture(
      emptyRegistration.copy(exampleMultipleQuestions = Some(ExampleMultipleQuestions("value 1", "value 2")))
    ) {
      val result: Future[Result] = controller.onPageLoad(NormalMode)(fakeRequest)

      status(result)          shouldBe OK
      contentAsString(result) shouldBe view(form.fill(ExampleMultipleQuestions("value 1", "value 2")), NormalMode)(
        fakeRequest,
        messages
      ).toString
    }
    ...
    "redirect to the next page when valid data is submitted" in new TestFixture() {
      ...
      val result: Future[Result] =
        controller.onSubmit(NormalMode)(
          fakeRequest.withFormUrlEncodedBody(("field1" -> "value 1"), ("field2" -> "value 2"))
        )
    ...
    "return a Bad Request and errors when invalid data is submitted" in new TestFixture() {
      ...
      val formWithErrors: Form[ExampleMultipleQuestions] = form.bind(Map("value" -> ""))
