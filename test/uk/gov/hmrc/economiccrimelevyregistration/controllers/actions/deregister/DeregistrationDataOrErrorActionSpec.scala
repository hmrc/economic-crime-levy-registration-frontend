  ...
  "refine" should {
    "transform an AuthorisedRequest into a DeregistrationDataRequest when data retrieval succeeds" in forAll {
      (internalId: String, groupId: String, deregistration: Deregistration) =>
        when(mockDeregistrationService.get(any())(any(), any()))
          .thenReturn(EitherT.fromEither[Future](Right(Some(deregistration))))
    ...
    "redirect to error page when data retrieval fails" in forAll { (internalId: String, groupId: String) =>
      ...
      await(result) shouldBe Left(Redirect(routes.NotableErrorController.alreadyDeregistered()))
    }
  }
}
