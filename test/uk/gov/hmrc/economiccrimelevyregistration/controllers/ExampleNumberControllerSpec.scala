...
class ExampleNumberControllerSpec extends SpecBase {

  val view: ExampleNumberView = app.injector.instanceOf[ExampleNumberView]

  val formProvider = new ExampleNumberFormProvider()
  ...
  class TestFixture(data: Registration = emptyRegistration) {
    val controller = new ExampleNumberController(
  ...
  "onPageLoad" should {
    ...
    "populate the view correctly when the question has previously been answered" in new TestFixture(
      emptyRegistration.copy(exampleNumber = Some(1))
    ) {
      val result: Future[Result] = controller.onPageLoad(NormalMode)(fakeRequest)
