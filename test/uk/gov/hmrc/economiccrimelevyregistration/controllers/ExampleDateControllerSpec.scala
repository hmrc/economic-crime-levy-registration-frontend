...
class ExampleDateControllerSpec extends SpecBase {

  val view: ExampleDateView = app.injector.instanceOf[ExampleDateView]

  val formProvider = new ExampleDateFormProvider()
  ...
  class TestFixture(data: Registration = emptyRegistration) {
    val controller = new ExampleDateController(
  ...
  "onPageLoad" should {
    ...
    "populate the view correctly when the question has previously been answered" in new TestFixture(
      emptyRegistration.copy(exampleDate = Some(validDate))
    ) {
      val result: Future[Result] = controller.onPageLoad(NormalMode)(fakeRequest)

      status(result)          shouldBe OK
      contentAsString(result) shouldBe view(form.fill(validDate), NormalMode)(fakeRequest, messages).toString
    }
