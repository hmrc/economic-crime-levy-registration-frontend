...
class DeregisterContactEmailFormProviderSpec extends StringFieldBehaviours {
  ...
  val requiredKey = "deregisterContactEmail.error.required"
  val lengthKey   = "deregisterContactEmail.error.length"

  val form = new DeregisterContactEmailFormProvider()()
  ...
  "value" should {
    ...
    "fail to bind an invalid email address" in forAll(
      ...
      result.errors.map(_.message) should contain("deregisterContactEmail.error.invalid")
    }
  }
}
